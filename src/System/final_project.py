...
class SimulatedConnection:
    ...
    def send(self, encrypted_message):
        """Simulates sending an encrypted message to the receiver."""
        decrypted_message = self.receiver.decrypt_message(encrypted_message.decode())
...
class ConsensusNode:
    def __init__(self, node_id):
        ...
        self.peers = {}  # Dictionary of peers {peer_id: SimulatedConnection}
        self.local_data = []  # Local data storage
    ...
    def send_shared_key(self, peer_public_key_pem):
        ...
        shared_key = os.urandom(32)  # Generate a 256-bit AES key
        ...
        while self.alive:
            for peer_id in self.peers.keys():
                try:
                    ...
                    self.peers[peer_id]["connection"].send(encrypted_message.encode())
    ...
    def receive_data(self, data):
        ...
        print(f"Node {self.node_id} stored data: {data}")
    ...
    def handle_incoming_message(self, encrypted_message, peer_id):
        ...
        self.receive_data(decrypted_message)
        self.replicate_data(decrypted_message)
    ...
    def leader_broadcast(self, message):
        if not self.is_leader:
            raise Exception(f"Node {self.node_id} is not the leader and cannot broadcast.")
        print(f"Leader Node {self.node_id} broadcasting: {message}")
        self.replicate_data(message)

    def detect_leader_failure(self):
        if self.leader_id and self.leader_id not in self.peers:
            print(f"Node {self.node_id} detected leader failure. Starting re-election.")
            self.start_election()

    def handle_node_join(self, new_node):
        print(f"Node {self.node_id} handling join request from Node {new_node.node_id}")
        self.add_peer(new_node.node_id, SimulatedConnection(sender=self, receiver=new_node))
        new_node.simulate_connection(self)
        self.replicate_existing_data(new_node)

    def replicate_existing_data(self, new_node):
        for data in self.local_data:
            encrypted_data = self.encrypt_message(data)
            new_node.receive_data(encrypted_data)

    def handle_node_leave(self, departing_node_id):
        if departing_node_id in self.peers:
            print(f"Node {self.node_id} removing Node {departing_node_id} from peers.")
            del self.peers[departing_node_id]

    def reintegrate_node(self, peers):
        print(f"Node {self.node_id} reintegrating into the system...")
        for peer_id, peer_info in peers.items():
            peer = peer_info["connection"].receiver
            self.simulate_connection(peer)
            for data in peer.local_data:
                if data not in self.local_data:
                    self.local_data.append(data)
        print(f"Node {self.node_id} successfully reintegrated.")
...
plt.title("Failure and Recovery Simulation Using Designed System")
plt.grid(True)
